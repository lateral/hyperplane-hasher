...
class TestHyperplaneHasherDDB(unittest.TestCase, HyperplaneHasherTestAbstract):
    ...
    def setUp(self):
        self.kvstore = DynamoDBAdapter(np.dtype('float32'), 'hh_test_table')
        HyperplaneHasherTestAbstract.setUp(self)

    def tearDown(self):
        [item.delete() for item in self.kvstore.table.scan()]
